...
void mattack::bite(monster *z, int index)
{
    ...
    if( uncanny || dodge_check(z, target) ){
        auto msg_type = target == &g->u ? m_warning : m_info;
        sfx::play_variant_sound( "mon_bite", "bite_miss", sfx::get_heard_volume(z->pos()), 0, sfx::get_heard_angle(z->pos()));
        target->add_msg_player_or_npc( msg_type, _("The %s lunges at you, but you dodge!"),
                                              _("The %s lunges at <npcname>, but they dodge!"),
                                    z->name().c_str() );
        if( !uncanny ) {
    ...
    if( dam > 0 ) {
        ...
        //~ 1$s is monster name, 2$s bodypart in accusative
        if ( target->is_player()) {
            sfx::play_variant_sound( "mon_bite", "bite_hit", sfx::get_heard_volume(z->pos()), 0, sfx::get_heard_angle(z->pos()));
